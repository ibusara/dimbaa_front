...
export class RefereeHistoryComponent implements OnInit {
  ...
  constructor(private router: Router, private refereeService: RefreeService) {}
  ...
  handleView(e: any) {
    this.router.navigate(['/dashboard/r/referee/form/details'], {
      queryParams: { id: e.number },
    });
  }

  handleUpload(e: any) {
    this.router.navigate(['/dashboard/r/referee/form/upload'], {
      queryParams: { id: e.number },
    });
  }

  getMatchList() {
    this.refereeService.listMatchEvent().subscribe((response: any) => {
      if (response) {
        this.table.data = response.match;
      }
    });
  }
}
